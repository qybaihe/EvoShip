// 真实蜂群运行 + 画布回放引擎:
// 先调用 /v1/chat/completions,再把 x_swarm_trace 映射到用户画出的拓扑上播放。
...
export function useFlowRunner() {
  ...
  async function replayGraphOnCanvas(graph: SwarmGraph, nodes: FlowNode[], edges: FlowEdge[], content: string, token: number, opts: { onExperience?: (kind: "in" | "out", count: number, nodeId?: string) => void; inheritedCount: number } = { inheritedCount: 0 }) {
    ...
    if (!orderedEvents.length) {
      ...
      if (node) {
        store.setBubble(node.id, shortSpeech(content || "真实蜂群返回了最终答案。"), "active");
        await sleep(1000);
    ...
    for (const event of orderedEvents) {
      ...
      if (graphEdge) {
        ...
        if (source && target) {
          ...
          if (text && event.kind === "revision") store.setBubble(target.id, text, "error");
        } else {
        ...
        await sleep(kind === "broadcast" ? 720 : 620);
  ...
  function fallbackBees(content: string, nodes: FlowNode[], order: string[]): SwarmBee[] {
    const summary = shortSpeech(content || "真实蜂群返回了最终答案。");
    const roles = order
  ...
  async function run(nodes: FlowNode[], edges: FlowEdge[], params: RunParams) {
    ...
    try {
      ...
      store.addLog("EvoShip", `真实蜂群返回 ${graphSize},实际策略:${actual}。`);
