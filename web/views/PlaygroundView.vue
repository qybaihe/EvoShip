...
async function runDemo() {
  ...
  toast.show("🎬 Demo 已就绪:无需 API Key,正在播放默认 Evo 蜂群完整链路…");
...
<template>
  <div class="pg-layout">
    ...
    <div class="canvas-wrap" @drop="onDrop" @dragover="onDragOver">
      <!-- 顶栏 -->
      <div class="pg-topbar">
      ...
      <div class="pg-runbar">
        <div class="run-meta">
          ...
          <span>{{ store.mode === "custom" ? "自定义编队用于真实 trace 可视化回放" : "默认预览按当前档位展开,运行后替换为后端真实 graph" }}</span>
        </div>
        ...
        <button v-if="!running" class="run-btn" @click="dispatch()" :disabled="!goal.trim()">
          派出真实蜂群
        </button>
      ...
      <div class="answer-panel" v-if="store.finalAnswer || store.errorMsg || store.playbackStatus !== 'idle'">
        ...
        <div v-else-if="running" class="answer-wait">正在调用后端蜂群,返回后会在画布上重放协作过程。</div>
            ...
            <div v-if="selectedAuditStep" class="audit-dialog">
              ...
              <div class="dialog-bubbles">
                ...
                <div v-if="selectedAuditStep.receiver" class="dialog-bubble receiver">
                  <label>流转确认</label>
                  <p>{{ selectedAuditStep.transferText }}</p>
                </div>
              </div>
            </div>
          </div>
