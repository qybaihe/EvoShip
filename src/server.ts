...
import { withMockModel, withModelProvider, type ModelProvider } from "./model.js";
...
app.post("/api/playground/swarm/run", async (req, res) => {
  ...
  const token = bearerToken(req);
  const caller = resolveApiCaller(req);
  if (!isDemo && !caller && isEvoShipApiKey(token)) {
    return res.status(401).json({ error: { message: "invalid EvoShip API key", type: "invalid_api_key" } });
  }

  const resolved = resolveModel(requested, isDemo ? null : caller);
  if (resolved.error) {
    return res.status(resolved.error.status).json({ error: { message: resolved.error.message, type: "invalid_request" } });
  }
  // 必须有用户 API key 才能调用
  ...
  try {
    ...
    const out = isDemo ? await withMockModel(run) : await withModelProvider(caller?.provider, run);
    // 成功完成:扣积分
    if (!isDemo && caller && userId) {
